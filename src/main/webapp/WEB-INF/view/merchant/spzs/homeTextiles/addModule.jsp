...
<title>编辑</title>
<%@include file="../../js.jsp"%>
<link rel="stylesheet" href="<%=basePath %>/resource/css/spzs/homeTextiles/addModule.css" />
...
function openImage4ModBgDiv(){
	$("#image4ModBg_div").css("display","block");
}
...
function closeImage4ModBgDiv(){
	$("#image4ModBg_div").css("display","none");
}
...
function uploadImage4(){
	if($("#image4Mod_div table td[class='file_td']").length>=5){
		alert("最多上传5张图片!");
		return false;
	}
	var uuid=createUUID();
	$("#uuid_hid4").val(uuid);
	$("#uploadFile4_div").append("<input type=\"file\" id=\"uploadFile4_inp\" name=\"file"+uuid+"\" onchange=\"showQrcodePic4(this)\"/>");
...
function deleteImage4(o){
	var td=$(o).parent();
	var uuid=td.attr("id").substring(7);
	$("#image4_div #list_div img[id='img"+uuid+"']").remove();
	td.remove();
	$("#uploadFile4_div input[type='file'][name='file"+uuid+"']").remove();
	
	var imageTab=$("#image4Mod_div table");
	var tdArr1=imageTab.find("td");
	imageTab.empty();
	for(var i=0;i<tdArr1.length;i++){
		var tdArr2=imageTab.find("td");
...
function showQrcodePic4(obj){
	...
	var fileHtml=file.prop("outerHTML");
	var tdHtml="<td class=\"file_td\" id=\"file_td"+uuid+"\">"
				+"<img class=\"delete_img\" alt=\"\" src=\"/GoodsPublic/resource/images/004.png\" onclick=\"deleteImage4(this);\">"
				+"<img class=\"item_img\" id=\"img"+uuid+"\" alt=\"\">"
				+fileHtml
			+"</td>";
    ...
    if(tdLength%4==0){
    	var tr=imageTab.find("tr").eq(imageTab.find("tr").length-1);
    	tr.append(tdHtml)
    	imageTab.append("<tr>"+$("#image4Mod_div table #upload_td").prop("outerHTML")+"</tr>");
    	tr.find("td[id^='upload_td']").remove();
    }
    else{
		imageTab.find("#upload_td").before(tdHtml);
    ...
    var windowURL = window.URL || window.webkitURL;
    var dataURL;
    var $img = $("#image4Mod_div table #img"+uuid);
			...
			<table>
				<tr>
					<td class="file_td" id="file_td1_1">
						<img class="delete_img" alt="" src="/GoodsPublic/resource/images/004.png" onclick="deleteImage1(this);">
						<img class="item_img" id="img1_1" alt="" src="/GoodsPublic/resource/images/spzs/8878b699bd21eeaeeaec254b5d4e6f79.png">
					</td>
					<td id="upload_td">
			...
			<table>
				<tr>
					<td class="file_td" id="file_td2_1">
						<img class="delete_img" alt="" src="/GoodsPublic/resource/images/004.png" onclick="deleteImage2(this);">
						<img class="item_img" id="img2_1" alt="" src="/GoodsPublic/resource/images/spzs/e3f0c2aa261ac2b652fbdca9eaba04f3.png">
						<!-- 
			...
			<table>
				<tr>
					<td class="file_td" id="file_td3_1">
						<img class="delete_img" alt="" src="/GoodsPublic/resource/images/004.png" onclick="deleteImage3(this);">
						<img class="item_img" id="img3_1" alt="" src="/GoodsPublic/resource/images/spzs/\bc40c19357bcf636e9ad6e71581686c4.png">
					</td>
					<td id="upload_td">
...
<div class="image4ModBg_div" id="image4ModBg_div">
	<div class="image4Mod_div" id="image4Mod_div">
		<div class="title_div">
			<span class="title_span">图片模块</span>
			<span class="close_span" onclick="closeImage4ModBgDiv();">关闭</span>
		</div>
		<div id="tab_div">
			<table>
				<tr>
					<td class="file_td" id="file_td4_1">
						<img class="delete_img" alt="" src="/GoodsPublic/resource/images/004.png" onclick="deleteImage4(this);">
						<img class="item_img" id="img4_1" alt="" src="/GoodsPublic/resource/images/spzs/\f078f4614735e89cd7608110115c6ad6.png">
					</td>
					<td id="upload_td">
						<img class="upload_img" alt="" src="/GoodsPublic/resource/images/005.png" onclick="uploadImage4();">
					</td>
				</tr>
			</table>
			<div class="uploadFile4_div" id="uploadFile4_div">
				<input type="file" id="file4_1" name="file4_1" onchange="showQrcodePic4(this)" />
			</div>
			<input type="hidden" id="uuid_hid4"/>
		</div>
		<div class="but_div" id="but_div">
			<div class="confirm_div" onclick="closeImage4ModBgDiv();">确&nbsp;认</div>
			<div class="cancel_div" onclick="closeImage4ModBgDiv();" onmousemove="changeButStyle(this,1);" onmouseout="changeButStyle(this,0);">取&nbsp;消</div>
		</div>
	</div>
</div>
...
<div class="middle_div" id="middle_div">
	...
	<div class="spxq_div" id="spxq_div">
		<table class="spxq_tab" id="spxq_tab">
			<tr class="head_tr">
				<td class="spxq_td" colspan="2">产品参数</td>
	...
	<div class="image4_div" id="image4_div">
		<div class="option_div" id="option_div" onmousemove="showOptionDiv(this);" onmouseout="hideOptionDiv(this);">
			<div class="but_div" id="but_div">
				<a onclick="openImage4ModBgDiv();">编辑</a>|
				<a onclick="deleteImage4Div();">删除</a>
			</div>
		</div>
		<div class="list_div" id="list_div" onmousemove="showOptionDiv(this);" onmouseout="hideOptionDiv(this);">
			<c:forEach items="${requestScope.image4List }" var="image4" varStatus="status">
			<img class="item_img" id="img4_1" alt="" src="${image4.value }">
			</c:forEach>
		</div>
	</div>
</div>
